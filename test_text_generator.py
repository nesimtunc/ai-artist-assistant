...
def test_text_to_tensor():
    ...
    assert torch.equal(
        result, expected_tensor), f"Expected {expected_tensor}, but got {result}"
...
def test_generate_text():
    ...
    result = generate_text(model, seed, length,
                           temperature, char_to_idx, idx_to_char)

    # Check if the generated text has the correct length
    assert len(result) == length + \
        len(
            seed), f"Expected a string of length {length + len(seed)}, but got {len(result)}"
